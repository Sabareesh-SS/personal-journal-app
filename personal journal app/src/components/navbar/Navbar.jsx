...
function Navbar() {
  ...
  const pages = [{ name: 'Home', path: '/home' }, { name: 'My Journals', path: '/myjournals' }, { name: 'Support', path: '/support' }];
    ...
    <AppBar position="fixed" sx={{ backgroundColor: '#f5f5f5', color: '#333' }}>
      <Container maxWidth="xl">
        <Toolbar disableGutters sx={{ justifyContent: 'space-between' }}>
          ...
          <Box sx={{ display: { xs: 'none', sm: 'flex' }, justifyContent: 'center', flexGrow: 1, gap: 3 }}>
            {pages.map((page) => (
              <Button key={page.name} onClick={() => navigate(page.path)} sx={{ my: 2, color: '#333', display: 'block' }}>
                {page.name}
              </Button>
            ))}
          </Box>
